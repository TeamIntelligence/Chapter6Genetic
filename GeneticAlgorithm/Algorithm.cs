...
{
    public class GeneticAlgorithm<IInd> : IGeneticAlgorithm<IInd>
    {
            ...
            Func<Tuple<IInd,IInd>,Tuple<IInd,IInd>> crossover, Func<IInd, double, IInd> mutation)
        {
            ...
            var asd = finalFitnesses.OrderByDescending(c => c);
            Console.WriteLine("Best fitness: {0}", asd.First());
            return currentPopulation.Select((individual, index) => new Tuple<IInd, double>(individual, finalFitnesses[index])).OrderByDescending(tuple => tuple.Item2).First().Item1;
        }
